...
//=========================================
// 2. LOGIKA POS & ABSENSI (IN-FILE LOGIC AGAR TIDAK ERROR INCLUDE)
//=========================================

// A. Logika Pemilihan Pos
if (isset($_POST['pilih_pos_id'])) {
    $_SESSION['current_pos_id'] = $_POST['pilih_pos_id'];
    header("Refresh:0"); exit;
}

// B. Cek Status Session Pos
...
if (!isset($_SESSION['current_pos_id'])) {
    ...
    if ($stmt_absen->get_result()->num_rows == 0) {
        $show_modal_absensi = true;
    }
}

// E. Tema Warna
$tipe_pos = $config['tipe_pos'] ?? 'motor'; 
$theme_color = ($tipe_pos == 'mobil') ? 'blue' : 'yellow';
$theme_bg    = ($tipe_pos == 'mobil') ? 'bg-blue-600' : 'bg-yellow-500';
$theme_text  = ($tipe_pos == 'mobil') ? 'text-blue-600' : 'text-yellow-600';
                        ...
                        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden relative">
                            ...
                            <div class="p-8 border-b border-gray-100">
                                <form id="formCariTiket">
                                    <label class="block text-sm font-bold text-primary mb-3 uppercase tracking-wide">
                                    ...
                                    <div class="flex shadow-md rounded-xl overflow-hidden group focus-within:ring-2 focus-within:ring-<?php echo $theme_color; ?>-500 transition-all">
                                        <input type="text" id="kode_input" name="kode_input" class="w-full px-6 py-5 border-2 border-gray-200 border-r-0 rounded-l-xl text-3xl font-extrabold text-gray-800 focus:outline-none placeholder-gray-300 uppercase tracking-widest" placeholder="SCAN TIKET..." required autocomplete="off">
                            ...
                            <div id="paymentDetails" class="p-8 hidden">
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                                    <div class="p-4 bg-gray-50 rounded-xl border border-gray-100"><p class="text-[10px] text-gray-500 uppercase font-bold tracking-wider">Plat Nomor</p><p id="detail_plat" class="text-xl font-black text-primary mt-1">-</p></div>
                                    <div class="p-4 bg-gray-50 rounded-xl border border-gray-100"><p class="text-[10px] text-gray-500 uppercase font-bold tracking-wider">Jenis</p><p id="detail_jenis" class="text-xl font-bold text-gray-800 mt-1 capitalize">-</p></div>
                                    <div class="p-4 bg-gray-50 rounded-xl border border-gray-100"><p class="text-[10px] text-gray-500 uppercase font-bold tracking-wider">Masuk</p><p id="detail_masuk" class="text-base font-medium text-gray-700 mt-1">-</p></div>
                                    <div class="p-4 bg-blue-50 rounded-xl border border-blue-100"><p class="text-[10px] text-blue-600 uppercase font-bold tracking-wider">Durasi</p><p id="detail_durasi" class="text-lg font-bold text-primary mt-1">-</p></div>
                                </div>
                                <form id="formPembayaran">
                                    ...
                                    <div class="bg-primary rounded-2xl p-6 text-white shadow-lg mb-8 relative overflow-hidden group">
                                        ...
                                        <div class="flex justify-between items-end relative z-10">
                                            <div><span class="block text-gray-300 text-sm font-medium mb-1 uppercase tracking-wide">Total Tagihan</span><span id="detail_biaya" class="text-5xl font-extrabold text-<?php echo $theme_color; ?>-400 tracking-tight">Rp 0</span></div>
                                            <i class="fas fa-wallet text-5xl text-white/10"></i>
                                        </div>
                                    </div>
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                                        <div><label class="block text-sm font-bold text-gray-700 mb-2">Uang Diterima (Rp)</label><input type="number" id="jumlah_bayar" name="jumlah_bayar" class="w-full pl-4 py-4 border-2 border-gray-300 rounded-xl text-2xl font-bold focus:ring-2 focus:ring-<?php echo $theme_color; ?>-500 focus:border-<?php echo $theme_color; ?>-500 transition outline-none" placeholder="0" required></div>
                                        <div><label class="block text-sm font-bold text-gray-700 mb-2">Kembalian (Rp)</label><input type="text" id="kembalian" name="kembalian" class="w-full pl-4 py-4 border-2 border-gray-200 bg-gray-100 rounded-xl text-2xl font-bold text-gray-500 outline-none cursor-not-allowed" readonly placeholder="0"></div>
                                    </div>
                                    <div class="flex gap-4">
                                        ...
                                        <button type="submit" id="btnProses" class="w-2/3 px-6 py-4 bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 text-xl shadow-lg transition flex items-center justify-center"><i id="iconProses" class="fas fa-print mr-3"></i> <span id="textProses">Bayar & Cetak</span><i id="spinnerProses" class="fas fa-spinner fa-spin ml-2 hidden"></i></button>
                                    </div>
                                </form>
                            </div>
                        ...
                        <div class="space-y-6">
                            <?php if ($config): ?>
                                <div class="bg-black rounded-2xl shadow-lg overflow-hidden border-4 border-gray-800 relative group">
                                    ...
                                    <?php include 'cctv/cctv_masuk.php'; ?>
                                </div>
                                <div class="bg-black rounded-2xl shadow-lg overflow-hidden border-4 border-gray-800 relative group">
                                    <div class="absolute top-3 left-3 z-10 bg-red-600 text-white text-[10px] px-2 py-0.5 rounded animate-pulse font-bold tracking-wider">LIVE</div>
                                    <?php include 'cctv/cctv_keluar.php'; ?>
                                </div>
                            <?php else: ?>
...
<script src="js/pos_parkir_logic.js"></script>